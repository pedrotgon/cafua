...
interface AdvisorsSnapshotData {
  advisors: string[];
  meetings: string[];
}
...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    console.log(
      "[ADK API] 🚀 Starting advisors snapshot request for userId:",
      userId
    );
    ...
    // 1. Create ADK session for the advisors_snapshot_agent agent
    const sessionUrl = `http://localhost:8085/apps/advisors_snapshot_agent/users/${userId}/sessions`;
    ...
    // 2. Send message to advisors_snapshot_agent agent to get advisors summary
    const agentRequest: ADKAgentRequest = {
      app_name: "advisors_snapshot_agent",
      user_id: userId,
      session_id: session.id,
      new_message: {
        role: "user",
        parts: [{ text: `Get advisors summary for ${userId}` }],
      },
      streaming: false,
    };

    console.log("[ADK API] 🤖 Sending request to advisors agent...");
    console.log("[ADK API] 🤖 Agent request URL: http://localhost:8085/run");
    console.log(
      "[ADK API] 🤖 Agent request body:",
      JSON.stringify(agentRequest, null, 2)
    );

    const agentResponse = await fetch(`http://localhost:8085/run`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const advisorsData: AdvisorsSnapshotData = {
      advisors: parsedData.advisors || [],
    ...
    console.error("[ADK API] ❌ ERROR in advisors snapshot API:", error);
